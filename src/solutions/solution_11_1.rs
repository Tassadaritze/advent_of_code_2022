...
impl Monkey {
    ...
    fn add(&mut self, item: usize) -> usize {
    ...
    fn multiply(&mut self, item: usize) -> usize {
        self.inspect_count += 1;
        item * match self.operation_value {
            Some(val) => val,
            None => item,
    ...
    for _ in 0_u16..10000 {
        for i in 0..monkeys.len() {
            ...
            while let Some(item) = monkeys[i].items.pop_front() {
                let mut item = match monkeys[i].operation {
                    Operation::Add() => monkeys[i].add(item),
                    Operation::Multiply() => monkeys[i].multiply(item),
                };
