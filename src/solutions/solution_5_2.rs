        ...
        match num {
            ...
            1 => {
                for row in part.trim().split('\n') {
                    ...
                    let move_at = stacks[command[1]].len() - command[0].parse::<usize>().unwrap();
                    let mut moving = stacks.get_mut(command[1]).unwrap().split_off(move_at);
                    stacks.get_mut(command[2]).unwrap().append(&mut moving);
                }
            }
            _ => panic!(),
        }
